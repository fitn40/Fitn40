...
# Auto URL parameter auto-login capture
...
if st.session_state.current_page == "login":
    ...
    player_input = st.text_input("Enter Profile Name:", placeholder="e.g., Lord Bobby")
...
# UI SCREEN 2: MAIN DASHBOARD
    ...
    if is_admin:
        ...
        with col_b1:
            if st.button("➕ Create Bet Offer", use_container_width=True, type="primary"):
                st.session_state.current_page = "new_bet"
                st.rerun()
        with col_b2:
            if st.button("📊 View Full Backend DB", use_container_width=True):
    ...
    # 1. Open Bets Section (Single Player Context)
    ...
    if not open_bets:
        ...
        for bet in open_bets:
            with st.container(border=True):
                creator_name = bet.get('Creator', 'Unknown')
                st.markdown(f"**{creator_name}** predicts **{bet.get('Prediction')}** in *{bet.get('Match_Name')}*")
                # REQ UPDATED: Single player display name structure for unmatched offers
                st.write(f"💰 **{creator_name}** Risks: {bet.get('Points')} pts | 🎁 Opponent Must Risk to Match: {bet.get('Opponent_Payout')} pts")
                ...
                if is_admin or str(bet.get('Creator', '')).lower() == st.session_state.player_name.lower():
                    if st.button(f"🗑️ Delete Bet Offer #{bet.get('Bet_ID')}", key=f"del_open_{bet.get('Bet_ID')}", type="secondary", use_container_width=True):
                        ...
                        st.toast("Bet Offer removed!", icon="🗑️")
                        st.rerun()

    st.markdown("---")
    
    # 2. Live Matched Bets Section (Both Players Explicitly Named)
    ...
    if not live_bets:
        ...
        for bet in live_bets:
            with st.container(border=True):
                c_name = bet.get('Creator', 'Unknown')
                o_name = bet.get('Opponent', 'Unknown')
                st.markdown(f"🔒 **{c_name}** 🆚 **{o_name}**")
                st.write(f"**Match:** {bet.get('Match_Name')} | **Prediction:** {c_name} chose *{bet.get('Prediction')}*")
                # REQ UPDATED: Incorporating names of both actual players into the dynamic layout string
                st.write(f"📊 **Stakes Layout:** **{c_name}** risks {bet.get('Points')} pts vs **{o_name}** risks {bet.get('Opponent_Payout')} pts")
                
                if is_admin:
                    if st.button(f"🗑️ Delete Live Bet #{bet.get('Bet_ID')}", key=f"del_live_{bet.get('Bet_ID')}", type="secondary", use_container_width=True):
                        updated_list = [b for b in combined_bets if int(b["Bet_ID"]) != int(bet["Bet_ID"])]
                        save_all_bets_permanently(updated_list)
                        st.toast("Live Bet deleted!", icon="🗑️")
                        st.rerun()
...
# REAL-TIME MASTER BACKEND DATA INTERFACE VIEW PANEL
elif st.session_state.current_page == "view_db":
    st.title("📊 Master Backend Database")
    st.caption("This table pulls live transaction logs from the application core storage.")
    
    if len(combined_bets) == 0:
        st.info("The active database is currently empty (nil).")
    else:
        df_db = pd.DataFrame(combined_bets)
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name="fit_n_40_payout_database.csv",
            mime="text/csv",
